...
function renderCurrentTaskList(taskArray) {
  ...
  if (taskArray.length === 0) {
    let notItem = document.createElement("li")
    notItem.classList.add("task-list__not-item")
    notItem.textContent = "Задач нет"
    currentTaskList.append(notItem)
    return
  }
...
function renderDoneTaskList(taskArray) {
  doneTaskList.innerHTML = ""
    ...
    // Провекра кол-ва задач в массиве
    if (taskArray.length === 0) {
      let notItem = document.createElement("li")
      notItem.classList.add("task-list__not-item")
      notItem.textContent = "Задач нет"
      doneTaskList.append(notItem)
      return
    }
